...
Cargo workspace (resolver 3) producing one binary, `rust-service-starter`. All metadata, dependencies and lints are inherited from the root `Cargo.toml` (`[workspace.package]`, `[workspace.dependencies]`, `[workspace.lints]`); member crates never pin versions themselves.
